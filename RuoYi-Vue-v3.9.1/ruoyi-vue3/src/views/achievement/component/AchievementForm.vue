<template>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                  ...
                  <el-col :span="24">
                    <el-form-item label="比赛" prop="competitionId">
                      ...
                      <div style="margin-top: 5px; line-height: 1.2">
                        <el-link type="primary" @click="goToCompetitionApply"
                          >比赛找不到？点击这里申请赛事！</el-link
                        >
                      </div>
                    </el-form-item>
                  </el-col>
                  ...
                  <el-col :span="12">
                    <el-form-item label="团队名称" prop="teamName">
                      <el-input
                        ...
                        placeholder="请输入团队名称(选填)"
                      />
                    </el-form-item>
                  </el-col>
                  <el-col :span="12" v-if="form.isReimburse === 1">
                    <el-form-item label="报名费" prop="fee">
                      <el-input v-model.trim="form.fee" placeholder="请输入金额"
                ...
                <el-divider content-position="left"
                  ><i class="el-icon-money"></i> 报销申请</el-divider
                >
                ...
                <el-table
                  ...
                  :data="samAchievementParticipantList"
                  border
                ...
                >
                  ...
                  >
                    <template #default="scope">
                      <el-tag
                        :type="scope.row.manager == 1 ? 'success' : 'info'"
                        >{{ scope.row.manager == 1 ? "是" : "否" }}</el-tag
                      >
                    </template>
                  </el-table-column>
                </el-table>
                ...
                <el-table
                  ref="advisorTable"
                  :data="samAchievementAdvisorList"
                  border
                  style="width: 100%"
                  :row-class-name="tableRowClassName"
                >
                        ...
                        >
                          <template #title>
                            ...
                            <el-button
                              ...
                              @click="handlePreUpload(item.name)"
                              >查看上传示例</el-button
                            >
                          </template>
                        </el-alert>
                          ...
                          <div v-if="form[item.prop]" class="custom-file-row">
                            ...
                            <div class="file-action">
                              ...
                              <el-button
                                ...
                                :disabled="false"
                                :icon="Download"
                                @click="handleDownload(form[item.prop])"
                                >下载</el-button
                              >
                              <el-button
                                v-if="!readOnly"
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            ...
            >
              ...
              >
                <template #default="scope">
                  <el-tag :type="scope.row.manager == 1 ? 'success' : 'info'">{{
  ...
  >
    <div style="height: 650px; display: flex; flex-direction: column">
      <el-alert
        title="请务必参考以下示例整理您的文件，确认无误后点击下方按钮解锁上传功能。"
        type="error"
        :closable="false"
        class="mb10"
        show-icon
  ...
  >
    <el-form
      ...
      :rules="addParticipantRules"
      label-width="80px"
    >
      <el-form-item label="学号" prop="studentId">
        <el-input
          v-model.trim="participantForm.studentId"
          placeholder="请输入学号后点击空白处"
          @blur="handleParticipantIdBlur"
          clearable
        />
      </el-form-item>
      <el-form-item label="姓名" prop="studentName">
        <el-input
          v-model.trim="participantForm.studentName"
          placeholder="自动带出，或手动输入"
          :disabled="!isParticipantNew"
        />
      </el-form-item>

      <template v-if="isParticipantNew">
        <el-alert
          title="未匹配到该学号，请完善下方信息完成录入"
          type="warning"
          show-icon
          :closable="false"
        ...
        <el-form-item label="班级" prop="className">
          <el-input
            v-model.trim="participantForm.className"
            placeholder="请输入班级"
          />
        </el-form-item>
        <el-form-item label="年级" prop="classYear">
          <el-input
            v-model.trim="participantForm.classYear"
            placeholder="请输入年级 (例如: 2022)"
          />
        </el-form-item>
      </template>
    </el-form>
    <template #footer>
      <div class="dialog-footer">
        <el-button type="primary" @click="submitAddParticipant"
          >确 定</el-button
        >
  ...
  >
    <el-form
      ref="addAdvisorRef"
      :model="advisorForm"
      :rules="addAdvisorRules"
      label-width="80px"
    >
      <el-form-item label="工号" prop="teacherId">
        <el-input
          v-model.trim="advisorForm.teacherId"
          placeholder="请输入工号后点击空白处"
          @blur="handleAdvisorIdBlur"
          clearable
        />
      </el-form-item>
      <el-form-item label="姓名" prop="teacherName">
        <el-input
          v-model.trim="advisorForm.teacherName"
          placeholder="自动带出，或手动输入"
          :disabled="!isAdvisorNew"
        />
      </el-form-item>

      <template v-if="isAdvisorNew">
        <el-alert
          title="未匹配到该工号，请完善下方信息完成录入"
          type="warning"
          show-icon
          :closable="false"
          style="margin-bottom: 15px"
...
const data = reactive({
  ...
  rules: {
    competitionId: [
      { required: true, message: "比赛不能为空", trigger: "change" },
    ],
...
const attachmentConfig = [
  ...
  },
  {
    label: "收款码",
    name: "receipt",
    prop: "fileReceiptCode",
    alert: "请上传用于接收报销款的收款码",
    type: "warning",
    condition: (f) => f.isReimburse === 1,
  },
];
...
watch(
  () => form.value.fileAward,
  (uuid) => loadSafePreview(uuid, "award")
);
watch(
  () => form.value.fileNotice,
  (uuid) => loadSafePreview(uuid, "notice")
);
watch(
  () => form.value.fileWork,
  (uuid) => loadSafePreview(uuid, "work")
);
watch(
  () => form.value.filePayment,
  (uuid) => loadSafePreview(uuid, "payment")
);
watch(
  () => form.value.fileInvoice,
  (uuid) => loadSafePreview(uuid, "invoice")
);
watch(
  () => form.value.fileReceiptCode,
  (uuid) => loadSafePreview(uuid, "receipt")
);
...
function loadDetail(id) {
  ...
  props
    ...
    .then((response) => {
      ...
      pendingAchievementId.value =
        d && d.achievementId != null
          ? String(d.achievementId)
          : pendingAchievementId.value;
