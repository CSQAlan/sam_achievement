...
function getCompetitionList() {
  request({
    ...
    mergeCompetitionOptions(list);
  }).catch(() => {
    // Keep current option so review page can still display selected competition.
  });
}

// 修改：获取届次列表 (带 competitionId 参数)
function getSessionList(competitionId) {
  const normalizedCompetitionId = normalizeId(competitionId);
  ...
  request({
    ...
    params: { competitionId: normalizedCompetitionId, pageNum: 1, pageSize: 100 }
  }).then(response => {
...
function loadDetail(id) {
  ...
  props.getFn(id).then(response => {
    ...
    d.competitionId = normalizeId(d.competitionId ?? d.competition_id);
    d.competitionName = d.competitionName || d.competition_name || "";
    d.sessionId = normalizeId(d.sessionId ?? d.session_id);
    form.value = d;
    ensureCurrentCompetitionOption(d);
