  ...
  <div class="app-container review-page">
    ...
    <div v-if="showBottomDock" class="review-fixed-dock">
      <div class="review-fixed-inner">
        <div
          class="audit-toolbar"
          :class="{ 'audit-toolbar--nav-only': !showAuditControls }"
        >
          <div class="audit-toolbar-main">
            <div class="nav-toolbar">
              <el-button class="nav-btn back-btn" plain @click="handleBack"
                >返回列表</el-button
              >
              <el-button class="nav-btn prev-btn" plain @click="handlePrev"
                >上一个</el-button
              >
              <el-button class="nav-btn next-btn" plain @click="handleNext"
                >下一个</el-button
              >
            </div>
            ...
            <div
              v-if="showAuditControls && showRejectReason"
...
const apiMap = {
  ...
  college_level_reviewed: {
    getFn: getCollege_level_reviewed,
    addFn: addCollege_level_reviewed,
    updateFn: updateCollege_level_reviewed,
  },
  school_level_unreviewed: {
    getFn: getSchool_level_unreviewed,
    addFn: addSchool_level_unreviewed,
    updateFn: updateSchool_level_unreviewed,
  },
  school_level_reviewed: {
    getFn: getSchool_level_reviewed,
    addFn: addSchool_level_reviewed,
    updateFn: updateSchool_level_reviewed,
  },
};

// ✅ 来源（由列表页点击审阅传 query.source）
const source = computed(() =>
  String(route.query.source || "college_level_unreviewed").toLowerCase()
);
const mode = computed(() => {
...
const currentApi = computed(
  () => apiMap[source.value] || apiMap.college_level_unreviewed
);
...
const currentAuditDict = computed(() => {
  if (source.value.startsWith("college"))
    return college_audit_status?.value || [];
  if (source.value.startsWith("school"))
    return school_audit_status?.value || [];
  return [];
});
...
const nextStatusOptions = computed(() => {
  ...
  if (
    source.value === "school_level_unreviewed" ||
    source.value === "school_level_reviewed"
  ) {
...
const reasonManagementTitle = computed(() => {
  return isCollegeSource.value ? "管理院级常用原因" : "管理校级常用原因";
});
const currentReasonDictType = computed(() => {
  return isCollegeSource.value ? "college_reason" : "school_reason";
});
...
function findRejectReasonOption(options = [], token) {
  ...
  if (!text) return null;
  return (
    (options || []).find((opt) => opt.value === text || opt.label === text) ||
    null
  );
}
...
const baseRejectReasonOptions = computed(() => {
  const dictItems = isCollegeSource.value
    ? college_reason?.value
    : school_reason?.value;
  return normalizeRejectReasonOptions(dictItems || []);
});

const currentRejectReasonOptions = computed(
...
function isStatusSelectable(status) {
  ...
  return (nextStatusOptions.value || []).some(
    (opt) => String(opt.value) === String(status)
  );
}
...
function normalizeLooseText(value) {
  ...
  if (text === LEGACY_AUTO_SCHOOL_REJECT_REASON)
    return AUTO_SCHOOL_REJECT_REASON;
  return text;
}
...
function findPageIndexById(id) {
  ...
  return pageGroupsRef.value.findIndex((group) =>
    group.some((item) => String(item) === normalizedId)
  );
}
...
function loadPageIds() {
  ...
  if (fromQuery.length > 0) {
    ...
    pageIdsRef.value = pageKey.value
      ? parsePageIds(readJson(`review_page_${pageKey.value}`, []))
      : [];
  }
  ...
  if (groupFromQuery.length > 0) {
    ...
    if (pageKey.value) {
      try {
        sessionStorage.setItem(
          `review_current_page_${pageKey.value}`,
          JSON.stringify(groupFromQuery)
        );
      } catch (e) {
        // ignore storage errors
      }
    ...
    currentPageIdsRef.value = pageKey.value
      ? parsePageIds(readJson(`review_current_page_${pageKey.value}`, []))
      : [];
  }

  const storedGroups = pageKey.value
    ? readJson(`review_page_groups_${pageKey.value}`, [])
    : [];
  pageGroupsRef.value = Array.isArray(storedGroups)
    ? storedGroups
        .map((group) =>
          parsePageIds(Array.isArray(group) ? group.join(",") : group)
        )
  ...
  if (
    (!currentPageIdsRef.value || currentPageIdsRef.value.length === 0) &&
    currentPageIndexRef.value >= 0
  ) {
    currentPageIdsRef.value = resolveCurrentPageIdsByIndex(
...
function persistPageIds() {
  ...
  try {
    sessionStorage.setItem(
      `review_page_${pageKey.value}`,
      JSON.stringify(pageIdsRef.value)
    );
    sessionStorage.setItem(
      `review_current_page_${pageKey.value}`,
      JSON.stringify(currentPageIdsRef.value)
    );
    sessionStorage.setItem(
      `review_page_groups_${pageKey.value}`,
      JSON.stringify(pageGroupsRef.value)
    );
  } catch (e) {
    // ignore storage errors
  }
...
function buildReviewQuery(targetId, targetPageIndex) {
  const fallbackPageNum =
    currentPageIndexRef.value >= 0 ? currentPageIndexRef.value + 1 : 1;
  const targetPageNum =
    targetPageIndex >= 0
      ? targetPageIndex + 1
  ...
  if (!pageKey.value && pageIdsRef.value.length > 0)
    query.pageIds = pageIdsRef.value.join(",");
  ...
  if (targetPageIndex >= 0) {
    ...
    if (targetPageIds.length > 0)
      query.currentPageIds = targetPageIds.join(",");
...
function navigateToReview(targetId, targetPageIndex) {
  return router.push({
    path: route.path,
    query: buildReviewQuery(targetId, targetPageIndex),
  });
}
...
function handlePrev() {
  ...
  const isFirstOfCurrentPage =
    currentPageIdsRef.value.length > 0 &&
...
function handleNext() {
  ...
  const isLastOfCurrentPage =
    currentPageIdsRef.value.length > 0 &&
    String(currentPageIdsRef.value[currentPageIdsRef.value.length - 1]) ===
      String(currentId) &&
...
const isCollegeReject = computed(
  () => isCollegeSource.value && String(selectedAuditStatus.value) === "1"
);
const isSchoolReject = computed(
  () => isSchoolSource.value && String(selectedAuditStatus.value) === "0"
);
...
function syncAuditFromForm() {
  ...
  if (isCollegeSource.value) {
    ...
    const status =
      ...
      form.schooi_review_result ??
      form.schoolReviewResult ??
      form.school_review_result;
    if (isStatusSelectable(status)) {
      selectedAuditStatus.value = String(status);
    } else {
      setDefaultSelected();
...
watch(baseRejectReasonOptions, (options) => {
...
function handleRejectReasonCommand(value) {
  ...
  const matched = findRejectReasonOption(
    currentRejectReasonOptions.value,
    selectedValue
  );
  const nextText = normalizeLooseText(
    matched?.label || matched?.value || selectedValue
  );
  if (!nextText) return;
...
async function handleAddReason() {
  ...
  try {
    const maxSort = Math.max(
      0,
      ...fullReasonList.value.map((r) => r.dictSort || 0)
    );
    await addData({
      dictLabel: text,
      dictValue: text,
...
async function submitAudit() {
  ...
  try {
    ...
    proxy.$modal?.msgSuccess?.(
      isCollegeSource.value ? "院级审核成功" : "校级审核成功"
    );
    await jumpToNextAfterAudit(id);
  } catch (e) {
    proxy.$modal?.msgError?.("审核失败（可能已归档/已推送或接口限制）");
    const errMsg = e?.response?.data?.msg || e?.message || "审核失败";
...
function handleReviewNavigationReady(event) {
  ...
  if (detail.pageKey && String(detail.pageKey) !== String(pageKey.value || ""))
    return;
  if (detail.source && String(detail.source) !== String(source.value || ""))
    return;
  loadPageIds();
  persistPageIds();
}

function notifyReviewResultApplied(
  achievementId,
  reviewStatus,
  rejectReasonText = ""
) {
  ...
  window.dispatchEvent(
    new CustomEvent(REVIEW_RESULT_APPLIED_EVENT, {
      detail: {
...
onMounted(() => {
  if (typeof window !== "undefined") {
    window.addEventListener(
      REVIEW_NAV_READY_EVENT,
      handleReviewNavigationReady
    );
  }
  loadCurrent();
});
