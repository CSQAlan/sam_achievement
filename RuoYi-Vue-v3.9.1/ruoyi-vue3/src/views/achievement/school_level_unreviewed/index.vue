<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['achievement:school_level_unreviewed:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['achievement:school_level_unreviewed:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['achievement:school_level_unreviewed:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['achievement:school_level_unreviewed:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="school_level_unreviewedList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['achievement:school_level_unreviewed:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['achievement:school_level_unreviewed:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="school_level_unreviewedRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="届次 " prop="sessionId">
...
<script setup name="School_level_unreviewed">
import { listSchool_level_unreviewed, getSchool_level_unreviewed, delSchool_level_unreviewed, addSchool_level_unreviewed, updateSchool_level_unreviewed } from "@/api/achievement/school_level_unreviewed"
...
const school_level_unreviewedList = ref([])
...
function getList() {
  loading.value = true
  listSchool_level_unreviewed(queryParams.value).then(response => {
    school_level_unreviewedList.value = response.rows
...
function reset() {
  ...
  proxy.resetForm("school_level_unreviewedRef")
}
...
function handleUpdate(row) {
  ...
  getSchool_level_unreviewed(_achievementId).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["school_level_unreviewedRef"].validate(valid => {
    if (valid) {
      form.value.samAchievementParticipantList = samAchievementParticipantList.value
      if (form.value.achievementId != null) {
        updateSchool_level_unreviewed(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addSchool_level_unreviewed(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除校级未审核编号为"' + _achievementIds + '"的数据项？').then(function() {
    return delSchool_level_unreviewed(_achievementIds)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('achievement/school_level_unreviewed/export', {
    ...queryParams.value
  }, `school_level_unreviewed_${new Date().getTime()}.xlsx`)
}
