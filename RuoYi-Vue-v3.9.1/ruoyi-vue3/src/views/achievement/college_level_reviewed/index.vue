<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['achievement:college_level_reviewed:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['achievement:college_level_reviewed:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['achievement:college_level_reviewed:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['achievement:college_level_reviewed:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="college_level_reviewedList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['achievement:college_level_reviewed:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['achievement:college_level_reviewed:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <pagination
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="college_level_reviewedRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="届次 " prop="sessionId">
        ...
        <el-table :data="samAchievementParticipantList" :row-class-name="rowSamAchievementParticipantIndex" @selection-change="handleSamAchievementParticipantSelectionChange" ref="samAchievementParticipant">
          ...
          <el-table-column label="$comment" prop="studentId" width="150">
            <template #default="scope">
              <el-input v-model="scope.row.studentId" placeholder="请输入$comment" />
            </template>
          </el-table-column>
...
<script setup name="College_level_reviewed">
import { listCollege_level_reviewed, getCollege_level_reviewed, delCollege_level_reviewed, addCollege_level_reviewed, updateCollege_level_reviewed } from "@/api/achievement/college_level_reviewed"

const { proxy } = getCurrentInstance()
const { achievement_category, group_type, award_rank, award_level_type, college_audit_status } = proxy.useDict('achievement_category', 'group_type', 'award_rank', 'award_level_type', 'college_audit_status')

const college_level_reviewedList = ref([])
...
const data = reactive({
  ...
  rules: {
    ...
    sessionId: [
      { required: true, message: "届次 不能为空", trigger: "blur" }
    ],
    category: [
...
function getList() {
  loading.value = true
  listCollege_level_reviewed(queryParams.value).then(response => {
    college_level_reviewedList.value = response.rows
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["college_level_reviewedRef"].validate(valid => {
    if (valid) {
      form.value.samAchievementParticipantList = samAchievementParticipantList.value
      if (form.value.achievementId != null) {
        updateCollege_level_reviewed(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addCollege_level_reviewed(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除院级已审核编号为"' + _achievementIds + '"的数据项？').then(function() {
    return delCollege_level_reviewed(_achievementIds)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('achievement/college_level_reviewed/export', {
    ...queryParams.value
  }, `college_level_reviewed_${new Date().getTime()}.xlsx`)
}
