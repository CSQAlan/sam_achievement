 ...
 *    - updateDept_unapproved：auditStatus 0 -> 1/2（未审页消失）
 *    - addDept_approved：归档到院级已审核（显示“驳回/通过”）
 *    - 若通过(2)：addSdept_unapproved：推送到校级未审核（auditStatus=2 待校级审核）
 *
 * 2) 校级未审核(sdept_unapproved)
 *    - updateSdept_unapproved：auditStatus 2 -> 0/1（未审页消失）
 *    - addSdept_approved：归档到校级已审核（显示“校级通过/驳回”）
 */
async function submitAudit() {
  ...
  try {
    ...
    if (source.value === "dept_unapproved") {
      ...
      // 2) 归档到院级已审核（避免主键冲突，id 置空）
      await addDept_approved({ ...baseForm, id: null, auditStatus: next });

      // 3) 院级通过(2) -> 推送到校级未审核（待校级审核=2）
      if (next === "2") {
        await addSdept_unapproved({ ...baseForm, id: null, auditStatus: "2" });
      }
    ...
    if (source.value === "sdept_unapproved") {
      // 1) 更新校级未审核状态（2 -> 0/1）
      await updateSdept_unapproved({ ...baseForm, id, auditStatus: next });

      // 2) 归档到校级已审核
      await addSdept_approved({ ...baseForm, id: null, auditStatus: next });
